...
internal class Rebalancer
{
    ...
    {
        /// health
        if (building.Health.HasValue && baseAddresses.TryGetValue("Buildings Health", out var aHealth))
        {
            var address = GetAddress<Building>(aHealth, key);
            WriteIfDifferent(address, building.Health, aHealth.Size, $"{key} Health");
        }
        /// housing
        if (building.Housing.HasValue && baseAddresses.TryGetValue("Buildings Housing", out var aHousing))
        {
            var address = GetAddress<Building>(aHousing, key, 4);
            WriteIfDifferent(address, building.Housing, aHousing.Size, $"{key} Housing");
        }
        /// cost
        if (building.Cost != null && building.Cost.Length > 0 && baseAddresses.TryGetValue("Buildings Cost", out var aCost))
        {
            var address = GetAddress<Building>(aCost, key, building.Cost.Length);
    ...
    {
        /// buy
        if (resource.Buy.HasValue && baseAddresses.TryGetValue("Resources Buy", out var aBuy))
        {
            var address = GetAddress<Resource>(aBuy, key);
            WriteIfDifferent(address, resource.Buy, aBuy.Size, $"{key} Buy");
        }
        /// sell
        if (resource.Sell.HasValue && baseAddresses.TryGetValue("Resources Sell", out var aSell))
        {
            var address = GetAddress<Resource>(aSell, key);
            WriteIfDifferent(address, resource.Sell, aSell.Size, $"{key} Sell");
        }
    }
    ...
    {
        /// speed
        if (unit.Speed.HasValue && baseAddresses.TryGetValue("Units Speed", out var aSpeed))
        {
